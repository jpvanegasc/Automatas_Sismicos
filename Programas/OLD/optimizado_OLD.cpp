		...
		void inicie(Crandom &ran64){
			for(int i=0; i<Lx; i++)
				for(int j=0; j<Ly; j++){
					...
					if(i == 0) juan[i][j] = 0;
					else if(i == Lx-1) juan[i][j] = 0;
					else if(j == 0) juan[i][j] = 0;
					else if(j == Ly-1) juan[i][j] =0;
			...
			for(int i=1; i<Lx-1; i++)
				for(int j=1; j<Ly-1; j++){
					if(juan[i][j] >= F_th){ // Si hay falla
						...
						else if(i != Lx-2) juan[i+1][j] += juan[i][j]*alpha; // Izquierda
						else if(j != 1) juan[i][j-1] += juan[i][j]*alpha; // Arriba
						else if(j != Ly-2) juan[i][j+1] += juan[i][j]*alpha; // Abajo
						// Borrar celda actual
